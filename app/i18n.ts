...
export function parsePreferences(value:string|null):{locale:Locale;theme:Theme}{try{const p=JSON.parse(value||'{}');return {locale:languages.some(x=>x.id===p.locale)?p.locale:'ru',theme:p.theme==='light'?'light':'dark'}}catch{return {locale:'ru',theme:'dark'}}}
